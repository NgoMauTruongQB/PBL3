...
--
-- Host: 127.0.0.1
-- Generation Time: May 05, 2023 at 03:21 AM
...
('NN1234', 'Trần Kim Hiếu', '0379123546', 'Vĩnh Linh, Quảng Trị', '012211122211', ''),
...
--
-- Table structure for table `adopter_user`
--

CREATE TABLE `adopter_user` (
  `userID` varchar(50) NOT NULL,
  `adopterID` varchar(50) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Dumping data for table `adopter_user`
--

INSERT INTO `adopter_user` (`userID`, `adopterID`) VALUES
('U002', 'NN1234');
...
('A002', 'NN1236', 'C11224', 1, '2023-02-27', NULL),
('A003', 'NN1235', 'C11000', 1, '2020-09-02', 'Bé theo gia đình mới đi Mỹ'),
('A004', 'NN1234', 'C16809743', 1, '2023-04-27', NULL);
...
('DN006', 500000, 1, '0765121212', '2023-04-24', 'HD1201', 'U16802482'),
('DN007', 5000000, 1, '039900212', '2023-01-24', 'HD16821360', 'U16807167'),
('DN008', 1000000, 1, '0915123121', '2023-02-14', 'HD16821362', 'U16802728'),
('DN009', 1000000, 1, '0354714107', '2023-05-01', 'HD16821363', 'U16801611');
...
('GT002', 'Trầ Kim Hiếu', '0354715121', 'Điện Phương, Điện Bàn, Quảng Nam', NULL),
...
--
-- Table structure for table `introducter_user`
--

CREATE TABLE `introducter_user` (
  `userID` varchar(50) NOT NULL,
  `introducterID` varchar(50) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Dumping data for table `introducter_user`
--

INSERT INTO `introducter_user` (`userID`, `introducterID`) VALUES
('U16825263', 'GT16818338'),
('U002', 'GT002');
...
('NN0001', 'C16809741', 'GT16819285', 1, '2023-04-25', NULL),
('NN0002', 'C22012', 'GT16818338', 1, '2023-04-26', NULL),
('NN0003', 'C16809743', '11122100', 1, '2023-03-26', NULL),
('NN0004', 'C11223', 'GT002', 0, '2023-04-27', NULL);
...
('ST12111', 'Trần Lê Minh', '2000-11-10', 'Nam', 'Bảo vệ', 1, '0914123123', 'truongngo2707@gmail.com', ''),
...
('ST16809465', 'Vương Nữ Thu Thuỷ', '2000-12-05', 'Nữ', 'Nhân viên chăm trẻ', 1, '084548223', 'nghia28112003@gmail.com', ''),
...
('U002', 'hieukhongsay', 'truongngo2707@gmail.com', 'hieu12345', 'Trần Kim Hiếu', 'user', 1),
...
('U16825263', 'trangduong', '102210047@sv1.dut.udn.vn', 'trangduong123', 'Dương Thị Trang', 'user', 1);
...
--
-- Indexes for table `adopter_user`
--
ALTER TABLE `adopter_user`
  ADD KEY `FK__user` (`userID`),
  ADD KEY `FK_adopter_user_adopter` (`adopterID`);
...
--
-- Indexes for table `introducter_user`
--
ALTER TABLE `introducter_user`
  ADD KEY `FK_introducter_user_user` (`userID`),
  ADD KEY `FK_introducter_user_introducter` (`introducterID`);
...
--
-- Constraints for table `adopter_user`
--
ALTER TABLE `adopter_user`
  ADD CONSTRAINT `FK__user` FOREIGN KEY (`userID`) REFERENCES `user` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  ADD CONSTRAINT `FK_adopter_user_adopter` FOREIGN KEY (`adopterID`) REFERENCES `adopter` (`apdoterID`) ON DELETE NO ACTION ON UPDATE NO ACTION;
...
--
-- Constraints for table `introducter_user`
--
ALTER TABLE `introducter_user`
  ADD CONSTRAINT `FK_introducter_user_introducter` FOREIGN KEY (`introducterID`) REFERENCES `introducter` (`introducterID`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  ADD CONSTRAINT `FK_introducter_user_user` FOREIGN KEY (`userID`) REFERENCES `user` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION;
